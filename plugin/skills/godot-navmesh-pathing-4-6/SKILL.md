...
description: >-
  ...
  when the entity's BASE class (e.g. `PushableEnemy`) owns `_physics_process` —
  ...
  frame 1", "guard freezes at spawn", "path is just [start]",
...
substrate that makes the agent actually move. Every snippet is the shape proven on
mercenary's `Guard` (`entities/enemy/guard.gd`) + `levels/outpost_alpha.gd` runtime bake, and
diceofate's `NavUtils.ensure_baked()`.
...
1. **Bake at RUNTIME in the level `_ready()` — with the LEVEL ROOT as the explicit
   ...
   no navmesh**, the destination snaps to origin `(0,0,0)`, `path_size == 0`, and every guard
   ...
   floor + walls + cover are always scanned (proven on mercenary `outpost_alpha.gd`):
   ...
   `godot-enemy-ai-headless-smoke`). Any new level that needs nav follows this pattern (mercenary
   `known_issues.md` → Navigation). The naive `bake_navigation_mesh()` is a one-line trap that cost
   a full day; do not teach or copy it.
...
2. **Make the nav map ready before a frame-1 agent reads it.** After a bake (or after spawning
   ...
   cures:
   - **Force-commit (diceofate `NavUtils.ensure_baked()`):** `NavigationServer3D.map_force_update(map_rid)`
     right after the bake so the map is queryable that frame.
   - **Defer one physics frame (mercenary):** set the first destination via `call_deferred`, or
     skip the finished-check for one tick (the one-tick guard, step 4) so the server gets its tick.

   Symptom this fixes: _"a spawned guard stands still until the player gets close."_
...
3. **Snap every destination ONTO the mesh.** An off-mesh point (a waypoint a hair above the
   ...
   returns the agent's own position and it never moves. Snap first (real `Guard.set_nav_destination`):
   ...
   func set_nav_destination(point: Vector3) -> void:
   	...
   	if map_rid.is_valid():
   		...
   		# path and get_next_path_position() returns the guard's own position.
...
global_position` the path is not resolved yet (or you're already there) — set zero velocity
   this tick and let the server resolve next frame (real `Guard.drive_nav_movement`):
...
5. **Fix the entity-velocity ordering when a BASE class owns `_physics_process`.** If the base
   (`PushableEnemy`) runs `move_and_slide()` in its own `_physics_process`, the FSM child nodes
   run AFTER the parent — so nav velocity a state set arrives a frame late or is overwritten to
   zero (guard sets velocity but never moves). Override `_physics_process` on the entity, advance
   the FSM MANUALLY first, THEN compose velocity, THEN slide (real `Guard._physics_process`):
   ...
   func _physics_process(delta: float) -> void:
   	...
   	move_and_slide()
   	_nav_desired_velocity = Vector3.ZERO   # reset: guard stops if the FSM set nothing this tick
   ```
...
6. **Promote the magic literals to archetype fields.** `REPATH_INTERVAL` (0.25) and the
   `shove_threshold` (the `0.5` horizontal-impulse cutoff, used twice) are tracked as tech-debt
   in `known_issues.md` → they belong on `EnemyArchetype` / an `@export`, not as bare literals
   in logic (data-driven rule).

7. **Silence `nav_debug` once movement is confirmed.** `guard.gd` `@export nav_debug` defaults
   `true` and prints `[NavDebug]` lines (path size, reachability, next-path-pos) — invaluable
   while a guard is frozen, noise once it moves. Flip it `false` per instance.

## Verification checklist

- [ ] The level `_ready()` runtime-bakes (`bake_navigation_mesh()`); an editor-baked-only build
      leaves every guard frozen. F5 → guards walk.
- [ ] A just-spawned guard moves within a frame or two, not only after the player approaches
...
- [ ] Nav velocity survives to `move_and_slide` (guard moves) — not overwritten to zero by the
      base `_physics_process` (FSM advanced FIRST).
- [ ] A pull/push still flings the guard (`apply_force` → impulse wins above `shove_threshold`);
      it resumes pathing after the impulse decays.
- [ ] No bare `0.25` / `0.5` tuning literals left in `aggro_state.gd` / `guard.gd` logic.
...
| Symptom                                                                                                                                          | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| All guards frozen — killable but never move, on a fresh level                                                                                    | Navmesh never baked: the 4.6 editor bake ships EMPTY. Runtime-bake in the level `_ready()` — but with the LEVEL ROOT as the explicit source node (next row), not the bare `bake_navigation_mesh()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Guards frozen at origin: `target (0,0,0)`, `path_size 0`, yet `nav non-null` / `map_rid valid` / `is_target_reachable` all true; bake WAS called | The level runs nested under `Main/LevelHost`, and `_nav_region.bake_navigation_mesh()` scans only the NavigationRegion3D's own children (`SOURCE_GEOMETRY_ROOT_NODE_CHILDREN`) — empty → 0 polygons. Parse explicitly with the level root: `NavigationServer3D.parse_source_geometry_data(nm, geo, self)` + `bake_from_source_geometry_data(nm, geo)` + re-assign + `map_force_update`. A standalone `test_*.tscn` hides it (floor is a child of the scan root when F5'd as main scene). Add a `polygon_count > 0` headless gate (`godot-enemy-ai-headless-smoke`) — the naive headless bake also yields 0 polys, so without it every fix falsely "passes". |
...
| Guard sets a velocity but never moves; nav velocity ends up 0                                                                                    | A base class (`PushableEnemy`) ran `move_and_slide()` before the FSM children. Override `_physics_process`, call `_fsm.advance(delta)` FIRST, then compose velocity + `move_and_slide()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Pull/push doesn't move the guard, or the guard rubber-bands while shoved                                                                         | Impulse below `shove_threshold` so nav overwrites it, or the threshold is too high. Compose: impulse wins above `shove_threshold`; promote the `0.5` literal to a field and tune.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `[NavDebug]` spam floods the console                                                                                                             | `guard.gd` `@export nav_debug` defaults `true`. Flip it `false` on the guard instances once movement is confirmed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
